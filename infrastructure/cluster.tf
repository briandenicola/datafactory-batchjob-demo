...
resource "null_resource" "pod_identity_keda" {
  depends_on = [
...
resource "null_resource" "pod_identity_batchjob" {
  depends_on = [
    null_resource.pod_identity_keda
  ]
  provisioner "local-exec" {
    command = "./aks-pod-identity-creation.sh"
    interpreter = ["bash"]
    ...
    environment = {
      ...
      CLUSTER_RG          = "${azurerm_resource_group.k8s.name}"
      SUBSCRIPTION_ID     = "${data.azurerm_client_config.current.subscription_id}"
      IDENTITY_NAME       = "${var.cluster_name}-batchjob-identity"
      IDENTITY_RG         = "${azurerm_resource_group.k8s.name}"
      NAMESPACE           = "default"
    }
  }
